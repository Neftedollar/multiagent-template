...
| Launched via `nessy -p` | **Autonomous** | You = orchestrator. Read `docs/process.md`, pick tasks from backlog. |
...
- `code/{{PROJECT_NAME}}/CLAUDE.md` — architecture, build, tests (Nessy shares Claude's config)
...
~/{{PROJECT_NAME}}/
├── code/
│   └── {{PROJECT_NAME}}/   ← main code repo (has its own CLAUDE.md)
├── docs/
...
│   └── ...
└── .claude/
    └── commands/            ← slash commands (Nessy reads .claude/ config)
```

- **Code**: `code/{{PROJECT_NAME}}/`. Has its own `CLAUDE.md` — read it when working with code.
- **Config**: Nessy shares the `.claude/` directory with Claude Code (commands, hooks, settings).
...
**Chief of Staff** — Nessy in this workspace (default). Coordinates, answers questions, suggests next steps.
