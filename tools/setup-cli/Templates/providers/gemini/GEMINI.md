...
| Launched via `gemini -p` | **Autonomous** | You = orchestrator. Read `docs/process.md`, pick tasks from backlog. |
...
- `code/{{PROJECT_NAME}}/GEMINI.md` — architecture, build, tests
...
~/{{PROJECT_NAME}}/
├── code/
│   └── {{PROJECT_NAME}}/   ← main code repo (has its own GEMINI.md)
├── docs/
...
- **Code**: `code/{{PROJECT_NAME}}/`. Has its own `GEMINI.md` — read it when working with code.
...
**Chief of Staff** — Gemini CLI in this workspace (default). Coordinates, answers questions, suggests next steps.
...
| Tier | Model | When |
|------|-------|------|
| **Strategic** | gemini-2.5-pro | PM, Architects, Security, Orchestrator |
| **Execution** | gemini-2.5-flash | Coder, Frontend, DevOps, Tech Writer, Marketing, Designer |
| **Validation** | gemini-2.5-pro | Reality Checker, Code Reviewer |
| **Routine** | gemini-2.0-flash | Data gathering, formatting, lookups |
...
- **Code**: all code changes in `code/{{PROJECT_NAME}}/`, read its GEMINI.md.
